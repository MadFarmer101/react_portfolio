describe('User can see list of education and job experience', () => {
    ...
    it('displays first education card', () => {
      cy.get('#info-2').within(() => {
        ...
        cy.get('.description').should('contain', 'Degree in Economics at Vern Business School, Zagreb, Croatia.');
      })
    });
    ...
    it('displays second education card', () => {
        cy.get('#info-3').within(() => {
          ...
          cy.get('.description').should('contain', "Learnig to code at Craft Academy, Stockholm, Sweden");
        })
      });
    ...
    it('displays first job experience', () => {
      cy.get('#info-4').within(() => {
        cy.get(('.image')).should('exist');
        cy.get('.ui.header').should('contain', 'BestDeal site');
